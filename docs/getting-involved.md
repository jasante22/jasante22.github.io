---
title: Getting Involved
description: All things to do with clubs, organizations, events, etc. and how to navigate them early in the semester
What this page will detail:
- Gobblerfest, Gobblerfair, Gobblerconnect (if it starts with Gobble, it's probably about organizations)
- different kinds of organizations
  - academic (orgs for your resume basically; this doesn't mean they can't also be fun)
    - design teams for engineers
    - marketing firms for business ppl
    - journals for English majors
    - etc.
  - sports
    - intramural
    - club
  - Greek Life
    - real greek orgs
    - business frats and the works
  - interest-based
    - baking club
    - acapella
    - VDR
    - etc.
- how to navigate the events (gobblerfair/fest)
  - how they work, where to find clubs you're interested in
    - check their insta pages
- tips to STAY involved in clubs
  - don't get discouraged and set reminders for yourself
  - getting a job 
---

# Getting Involved
Leaving your hometown for college can be hard and stressful, especially when you're surrounded by people you don't know! With so many new faces and so many opportunities at your fingertips, how do you find where you belong? The answer is...clubs!

Clubs, sports, Greek life, and all the other extracurricular activities at Tech are wonderful options to find your niche. So...where do you start?

## Finding Student Organizations
There are approximately 800 student orgs on campus to choose from. While this is great for variety, it can be overwhelming to pick from that large of a pool. 